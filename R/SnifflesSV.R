...
Vcf2GRanges <- function(vcfFile) {
    ...
    karyo <- GRanges(seqnames = as.factor(getFIX(vcf)[, "CHROM"]), ranges = IRanges(start = as.numeric(getFIX(vcf)[, "POS"]), end = as.numeric(getFIX(vcf)[, 
        "POS"])))
...
snifflesKaryogram <- function(vcfFile) {
    ...
    if (length(unique(as.character(seqnames(karyo)))) == 1) {
        ...
        if (unique(gtools:::mixedsort(as.character(seqnames(karyo)))) == "4") {
            ...
            seqlengths(karyo) <- c(248956422, 242193529, 198295559, 190214555, 181538259, 170805979, 159345973, 145138636, 138394717, 133797422, 
                135086622, 133275309, 114364328, 107043718, 101991189, 90338345, 83257441, 80373285, 58617616, 64444167, 46709983, 50818468, 
                156040895, 57227415)
        }
    }
    
    karyogram <- autoplot(karyo, layout = "karyogram", aes_string(color = "SVTYPE", fill = "SVTYPE"), main = "Karyogram showing location and type of structural variations") + 
...
svLengthDistribution <- function(vcfFile) {
    ...
    plot <- ggplot(as.data.frame(mcols(sdata)), aes_string(x = "SVLEN", fill = "SVTYPE")) + geom_histogram(bins = 70) + scale_x_log10() + 
        scale_fill_brewer(palette = "Set1") + labs(title = "Plot showing frequency of structural variant length against log10 length") + 
