...
cas9ExecutiveSummary <- function() {
  ...
  depletionLabel = paste0(round(Hmisc::wtd.quantile(ontargetUniverse$dmean, probs=c(0.5), weight=IRanges::width(ontargetUniverse)) / Hmisc::wtd.quantile(as.numeric(backgroundUniverse$dmean), probs=c(0.5), weight=as.numeric(IRanges::width(backgroundUniverse))), digits=1), " X")

  execsummPlot <- infoGraphicPlot4(identifier=paste0(study, "_enrichment_info"),
                                   panelA=c(key="Throughput",           value=cas9Throughput, icon="fa-calculator"),
                                   panelB=c(key="reads on target",      value=ontargetLabel, icon="fa-cut"),
                                   panelC=c(key="mean target coverage", value=meanCovLabel, icon="fa-map"),
...
cas9MappingByGenomicSegment <- function() {
  ...
  collateMappingCharacteristics <- function(bamFile, unmappedBamFile=NA) {
    suppressWarnings(if ( !methods::is(unmappedBamFile, "data.frame") && is.na(unmappedBamFile)) { unmappedBamFile <- data.frame(width=numeric(), quality=numeric()) })
  ...
  targettable <- kable(summary.df, format="html", col.names=rep(" ", ncol(summary.df)), caption="Table summarising global mapping characteristics ranked by on-target, target-flanking and off-target", booktabs=TRUE, table.envir='table*', linesep="", escape = FALSE)  %>%
...
cas9TargetPerformanceTable <- function() {
  ...
  bygene <- cbind(names(ontargetUniverse),
                  ...
                  scales::comma_format()(ontargetUniverse$rstart),
                  scales::comma_format()(ontargetUniverse$basesstart),
                  scales::comma_format()(round(ontargetUniverse$meanreadlen, digits=2)),
  ...
  candidate_table <- kable(bygene, format="html", caption="Table summarising target mapping for pre-defined regions of interest", booktabs=TRUE, table.envir='table*', linesep="", escape = FALSE)  %>%
    kable_styling(c("striped", "condensed")) %>%
...
cas9SingleTargetPlot <- function(geneName, delta=0) {
  ...
  suppressWarnings(plot <- ggplot(covData)+
                     ...
                     geom_line(aes_string(x="start",y="binned_cov"), size=0.5, colour=brewer.pal(6, "Paired")[2])+
...
cas9WriteExcelOffTarget <- function() {
  ...
  writexl::write_xlsx(as.data.frame(offtargetUniverse)[,c(1,2,3,4,6,8,14,16,23)],
                      path = offtarget.meta)
}
...
cas9MultiGeneCoveragePanel <- function(colMax=4) {
  ...
  suppressWarnings(megadepthplot <- ggplot(covData, aes_string("pos", "binned_cov")) +
                     geom_hline(yintercept=(wga.cov * offtarget_level), colour="#E69F00") +
                     ...
                     xlab("Position across target region (kb)") + ylab("Depth of Coverage (X)") +
                     labs(title="Plot showing depth of coverage vs position for target regions") +
...
cas9CoverageTypeOverChromosomes <- function() {
  ...
  suppressWarnings(plot <- ggplot(targetMelt, aes_string("chromosome", "value")) +
                     geom_col(aes_string(fill="variable")) +
                     scale_y_continuous(labels = scales::comma) +
                     ylab("Number of bases (nt)") +
                     labs(title="Barchart showing number of references bases assigned as ontarget,\nofftarget or background") +
...
cas9OffTargetKaryogram <- function() {
  ...
  if (length(unique(as.character(GenomeInfoDb::seqnames(offtargetUniverse))))==1) {
    ...
    if (unique(gtools::mixedsort(as.character(GenomeInfoDb::seqnames(offtargetUniverse))))=="4") {
      ...
      GenomeInfoDb::seqlengths(offtargetUniverse) <- c(248956422, 242193529, 198295559, 190214555, 181538259, 170805979, 159345973, 145138636, 138394717, 133797422, 135086622, 133275309, 114364328, 107043718, 101991189, 90338345, 83257441, 80373285, 58617616, 64444167, 46709983, 50818468, 156040895, 57227415)
    }
  }
...
cas9OffTargetTable <- function() {
  ...
  offttable <- kable(offtargtop, format="html", caption="Table summarising the location and characteristics for the off-target regions with the highest depth-of-coverage", booktabs=TRUE, table.envir='table*', linesep="", escape = FALSE)  %>%
    kable_styling(c("striped", "condensed")) %>%
