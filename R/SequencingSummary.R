...
importSequencingSummary <- function(seqsum) {
    # downsample performed with cat lambda_sequencing_summary.txt | awk ' BEGIN {srand()} {print rand() ' ' $0}' | sort | head -5 | sed
    # 's/[^ ]* //'
    ...
    # passes_filtering is a useful flag; but there are examples of sequencing_summary.txt where this is not present -
...
SequencingSummaryPassGauge <- function(seqsum = NA) {
    ...
    df <- df %>% dplyr::mutate(group = ifelse(df$percentage < 0.6, "red", ifelse(df$percentage >= 0.6 & df$percentage < 0.8, "orange", "green")), 
        label = paste0(df$percentage * 100, "%"))
    
    title = "Percentage of reads\npassing QC filter"
    
    gaugePlot <- ggplot(df, aes_string(fill = "group", ymax = "percentage", ymin = 0, xmax = 2, xmin = 1)) + geom_rect(aes(ymax = 1, ymin = 0, 
        xmax = 2, xmin = 1), fill = "#ece8bd") + geom_rect() + coord_polar(theta = "y", start = -pi/2) + xlim(c(0, 2)) + ylim(c(0, 2)) + 
        guides(fill = FALSE) + guides(colour = FALSE) + theme_void() + theme(strip.background = element_blank(), strip.text.x = element_blank()) + 
        geom_text(aes_string(x = 0, y = 0, label = "label"), size = 13) + geom_text(aes_string(x = 1.5, y = 1.5, label = "title"), size = 11) + 
        scale_fill_manual(values = c(red = "#C9146C", orange = "#DA9112", green = "#129188")) + scale_colour_manual(values = c(red = "#C9146C", 
...
SequencingSummaryChannelActivity <- function(seqsum = NA, platform = NA) {
    ...
    activityPlot <- ggplot(channelMap, aes_string(x = "row", y = "col", fill = "count")) + geom_tile() + geom_text(data = channelMap, aes_string(x = "row", 
        y = "col", label = "count", color = "count"), show.legend = FALSE, size = 2.5) + scale_x_discrete(breaks = NULL) + scale_y_discrete(breaks = NULL) + 
        coord_equal() + scale_fill_gradientn(colours = hm.palette(100)) + scale_color_gradient2(low = hm.palette(100), high = hm.palette(1)) + 
        theme(axis.text.x = element_text(angle = 90, hjust = 1)) + labs(title = "Channel activity plot showing number of reads per flowcell channel") + 
        theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.title.x = element_blank(), 
            axis.title.y = element_blank(), legend.position = "bottom", legend.key.width = unit(5.6, "cm"))
...
getBinBreaks <- function(seqsum) {
    # pick a friendly upper limit to render sequence lengths into a histogram here we're aiming for a robustly rounded up 97.5 quantile of
    # the data (skip a few outliers ...)
...
SequencingSummaryWeightedReadLength <- function(seqsum) {
    ...
    weightedReadLengths <- ggplot(binnedBaseMelt, aes_string(x = "length", fill = "variable", y = "value")) + geom_bar(stat = "identity") + 
        xlab("Read length\n") + ylab("Number of bases sequenced\n") + scale_fill_manual("QC", values = c(fail = brewer.pal(6, "Paired")[1], 
        pass = brewer.pal(6, "Paired")[2])) + scale_x_continuous(limits = c(-breakVal, upperLimit), breaks = pretty(passedSeqs$sequence_length_template, 
        ...
        fill = "QV filter") + geom_vline(xintercept = N50, size = 1) + annotate("text", x = N50, y = max(passedBinnedBases + failedBinnedBases), 
        label = " N50", hjust = 0, colour = "SteelBlue") + geom_vline(xintercept = passedMeanLength, size = 1) + annotate("text", x = passedMeanLength, 
        y = max(passedBinnedBases + failedBinnedBases), label = " Mean", hjust = 0, colour = "SteelBlue")
...
SequencingSummaryReadLengthHistogram <- function(seqsum) {
    ...
    lengthHistogram <- ggplot(binnedReadMelt, aes_string(x = "length", fill = "variable", y = "value")) + geom_bar(stat = "identity") + 
        xlab("Read length\n") + ylab("Number of reads\n") + scale_fill_manual("QC", values = c(fail = brewer.pal(6, "Paired")[1], pass = brewer.pal(6, 
        "Paired")[2])) + scale_x_continuous(limits = c(-breakVal, upperLimit), breaks = pretty(passedSeqs$sequence_length_template, n = 40)) + 
        theme(axis.text.x = element_text(angle = 90, hjust = 1)) + labs(title = "Histogram showing distribution of read lengths across quality passing sequences", 
        fill = "QV filter") + geom_vline(xintercept = N50, size = 1) + annotate("text", x = N50, y = max(passedBinnedReads + failedBinnedReads), 
        label = " N50", hjust = 0, colour = "SteelBlue") + geom_vline(xintercept = passedMeanLength, size = 1) + annotate("text", x = passedMeanLength, 
        y = max(passedBinnedReads + failedBinnedReads), label = " Mean", hjust = 0, colour = "SteelBlue")
...
SequencingSummaryReadQualityHistogram <- function(seqsum) {
    qdist <- ggplot(seqsum, aes_string(x = "mean_qscore_template", fill = "passes_filtering")) + geom_histogram(breaks = seq(from = 0, to = 15, 
        by = 0.1)) + scale_fill_manual(name = "QC", values = c(`TRUE` = brewer.pal(6, "Paired")[2], `FALSE` = brewer.pal(6, "Paired")[1]), 
        labels = c("pass", "fail"), breaks = c("TRUE", "FALSE")) + labs(title = "Plot showing distribution of quality scores across all reads") + 
...
SequencingSummaryReadLengthQualityDensity <- function(seqsum, binFilter = 5, qcThreshold = 7) {
    ...
    qldensityplot <- ggplot(lq_dens_counts) + geom_bin2d(aes_string("x", "y", fill = "count"), stat = "identity") + scale_fill_distiller(palette = "Blues", 
        trans = "reverse") + geom_hline(yintercept = qcThreshold, size = 1) + xlab("log10(read length)") + ylab("read mean quality") + scale_x_continuous(breaks = c(1, 
        2, 3, 4, 5), labels = c("10", "100", "1000", "10,000", "100,000")) + annotation_logticks(base = 10, sides = "b", scaled = TRUE) + 
...
getTemporalDataset <- function(seqsum, sampleIntervalMinutes, breaks, binass) {
    ...
    binnedTemporalDataPerHour <- data.frame(cbind(time = breaks, pass = unlist(lapply(seq(breaks), mergeItPerHour, binnedAssignments = binass, 
        filter = TRUE)), fail = unlist(lapply(seq(breaks), mergeItPerHour, binnedAssignments = binass, filter = FALSE))))
...
SequencingSummaryTemporalThroughput <- function(seqsum, scaling = 1, sampleHours = 48, sampleIntervalMinutes = 60) {
    ...
    plot <- ggplot(binnedTemporalDataPerHour, aes_string("time")) + geom_line(aes(y = binnedTemporalDataPerHour$fail, colour = "fail"), 
        size = 1) + geom_line(aes(y = binnedTemporalDataPerHour$pass, colour = "pass"), size = 1) + scale_color_manual(name = "QV", values = c(fail = brewer.pal(6, 
        "Paired")[1], pass = brewer.pal(6, "Paired")[2])) + xlab("Time (hours)") + ylab("Gigabases sequenced per hour") + labs(title = "Plot showing sequence throughput against time")
...
SequencingSummaryCumulativeBases <- function(seqsum, scaling = 1, sampleHours = 48, sampleIntervalMinutes = 60) {
    ...
    cumulativePlot <- ggplot(binnedTemporalDataPerHour, aes_string("time")) + geom_line(aes(y = cumsum(binnedTemporalDataPerHour$fail), 
        colour = "fail"), size = 1) + geom_line(aes(y = cumsum(binnedTemporalDataPerHour$pass), colour = "pass"), size = 1) + scale_color_manual(name = "QV", 
        values = c(fail = brewer.pal(6, "Paired")[1], pass = brewer.pal(6, "Paired")[2])) + geom_segment(x = T50$minimum, y = 0, xend = T50$minimum, 
        yend = base50, colour = "darkgray", size = 1) + geom_segment(x = 0, y = base50, xend = T50$minimum, yend = base50, colour = "darkgray", 
        size = 1) + annotate("text", x = T50$minimum, y = base50, label = " T50", vjust = 1, hjust = 0, colour = "SteelBlue") + geom_segment(x = T90$minimum, 
        y = 0, xend = T90$minimum, yend = base90, colour = "darkgray", size = 1) + geom_segment(x = 0, y = base90, xend = T90$minimum, yend = base90, 
        colour = "darkgray", size = 1) + annotate("text", x = T90$minimum, y = base90, label = " T90", vjust = 1, hjust = 0, colour = "SteelBlue") + 
        xlab("Time (hours)") + ylab("Number of bases sequenced (Gigabases)") + labs(title = "Plot showing cumulative bases sequenced against time")
...
SequencingSummaryT50 <- function(seqsum, t = 0.5, scaling = 1, sampleHours = 48, sampleIntervalMinutes = 60) {
    ...
    T50 <- optimize(function(t0) abs(targetInterpolate(t0) - base50), interval = range(binnedTemporalDataPerHour[acquireTimePoints, "time"]))
...
SequencingSummaryCumulativeReads <- function(seqsum, scaling = 1, sampleHours = 48, sampleIntervalMinutes = 60) {
    ...
    binnedTemporalDataReadsPerHour <- data.frame(cbind(time = breaks, pass = unlist(lapply(seq(breaks), mergeItReadsPerHour, binnedAssignments = binass, 
        filter = TRUE)), fail = unlist(lapply(seq(breaks), mergeItReadsPerHour, binnedAssignments = binass, filter = FALSE))))
    ...
    cumulativePlot <- ggplot(binnedTemporalDataReadsPerHour, aes_string("time")) + geom_line(aes(y = cumsum(binnedTemporalDataReadsPerHour$fail), 
        colour = "fail"), size = 1) + geom_line(aes(y = cumsum(binnedTemporalDataReadsPerHour$pass), colour = "pass"), size = 1) + scale_color_manual(name = "QV", 
        values = c(fail = brewer.pal(6, "Paired")[1], pass = brewer.pal(6, "Paired")[2])) + xlab("Time (hours)") + ylab("Number of reads sequenced (Millions)") + 
...
SequencingSummarySpeedPlot <- function(seqsum, scaling = 1, sampleHours = 48, sampleIntervalMinutes = 60) {
    ...
    speedplot <- ggplot(speedTime, aes_string(x = "segment", y = "rate", group = "segment")) + geom_boxplot(fill = "steelblue", outlier.shape = NA) + 
        scale_x_continuous(name = "Time (hours)") + ylab("Sequencing rate (bases per second)") + labs(title = "boxplot showing distribution of translocation speed against time")
...
SequencingSummaryActiveChannelPlot <- function(seqsum, scaling = 1, sampleHours = 48, sampleIntervalMinutes = 60) {
    ...
    binnedTemporalChannels <- data.frame(time = breaks, channels = unlist(lapply(seq(breaks), mergeActiveChannels, binnedAssignments = binass)))
    
    binnedTemporalChannels$time <- binnedTemporalChannels$time/60/60
    
    activityPlot <- ggplot(binnedTemporalChannels, aes_string("time")) + geom_step(aes_string(y = "channels"), size = 1, colour = "Steelblue") + 
        xlab("Time (hours)") + ylab("Number of channels producing data") + labs(title = "Plot showing number of functional channels against time")
    return(ggplot2handler(activityPlot))
}
...
SequenceSummaryExecutiveSummary <- function(seqsum, flowcellId = "undefined") {
    ...
    infoFile1 <- infoGraphicPlot3(identifier = "ExecutiveSummaryValueBoxes", panelA = c(value = "flowcell", key = flowcellId, icon = "fa-qrcode"), 
        panelB = c(value = readCount, key = "Reads produced", icon = "fa-filter"), panelC = c(value = gigabases, key = "gigabases called", 
            icon = "fa-file-text-o"))
...
SequenceSummaryBasicInfoPlot <- function(seqsum) {
    ...
    # N50 length is the length of the shortest contig such that the sum of contigs of equal length or longer is at least 50% of the total
    ...
    infoFile2 <- infoGraphicPlot5(identifier = "SequenceCharacteristicValueBoxes", panelA = c(value = (scales::comma_format())(passedMeanLength), 
        key = "Mean Read Length (nt)", icon = "fa-bar-chart"), panelB = c(value = (scales::comma_format())(N50), key = "N50", icon = "fa-play"), 
        panelC = c(value = passedMeanQ, key = "Mean Read Quality (QV)", icon = "fa-area-chart"), panelD = c(value = failedMeanQ, key = "Mean Failed QV", 
            icon = "fa-bug"), panelE = c(value = longestRead, key = "Longest Read", icon = "fa-sort"))
    return(infoFile2)
}
