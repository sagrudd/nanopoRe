...
plotDepthOfCoverageMegablock <- function(coverageData, colMax = 4) {
    ...
    legendDF <- data.frame(x = Inf, y = Inf, lab = plotLegend, row = unlist(lapply(seq_len(plotCols), rep, times = colMax))[seq_along(plotLegend)],
        col = rep(seq(1, colMax), length.out = length(plotLegend)))

    suppressWarnings(megadepthplot <- ggplot(coverageDF, aes_string("start", "binned_cov")) + geom_hline(yintercept = meanCov, size = 0.3,
        colour = "red") + geom_line(colour = brewer.pal(6, "Paired")[2]) + facet_grid(rows = vars(row), cols = vars(col), shrink = TRUE) +
        coord_cartesian(ylim = c(0, ylimit)) + scale_x_continuous(labels = comma) + theme(axis.text.x = element_text(angle = 90, hjust = 1),
        strip.text.y = element_blank(), strip.text.x = element_blank()) + xlab("Position along chromosome (Mb)") + ylab("Depth of Coverage (X)") +
        labs(title = "Plot showing depth of coverage vs position for chromosomes mapped") + geom_text(aes_string("x", "y", label = "lab"),
...
plotOverallCovHistogram <- function(bamFile, segments = 50) {
    ...
    plot <- ggplot(binnedDf, aes_string(x = "coverage", y = "bases")) + geom_vline(xintercept = meanCov, size = 0.3, colour = "red") + geom_bar(stat = "identity",
        fill = brewer.pal(6, "Paired")[2]) + scale_y_continuous(labels = comma) + labs(title = "Histogram showing distribution of depth-of-coverage across genome") +
