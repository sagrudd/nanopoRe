...
isEnrichmentAnalysisComplete <- function(resultsDir=NULL) {
    ...
    mappingResultsFile <- file.path(resultsDir, paste0(getCachedYAMLValue(field ="study_name"), "_mapping_results", ".Rdata"))
    aggregatedCovFile <- file.path(resultsDir, paste0(getCachedYAMLValue(field ="study_name"), "_aggregated_coverage", ".Rdata"))
...
enrichmentExecutiveSummary <- function() {
    ...
    backgroundUniverse <- getCachedObject("backgroundUniverse", enrichmentEnvironment)
    unmappedReads <- getCachedObject("unmappedReads", enrichmentEnvironment)
    offtargetUniverse <- getCachedObject("offtargetUniverse", enrichmentEnvironment)
    targetproximalUniverse <- getCachedObject("targetproximalUniverse", enrichmentEnvironment)
    ontargetUniverse <- getCachedObject("ontargetUniverse", enrichmentEnvironment)
