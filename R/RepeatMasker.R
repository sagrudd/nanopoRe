...
# hg19 - Feb 2009 - RepeatMasker open-4.0.5 - Repeat Library 20140131
...
RepeatMaskerGR <- function(RepeatMaskerFile=NULL, RepeatMaskerSRC="http://www.repeatmasker.org/genomes/hg19/RepeatMasker-rm405-db20140131/hg19.fa.out.gz", force=FALSE) {
  ...
  RepeatMaskerGRData <- file.path(getRpath(),
                                  paste0(sub("\\.[^.]*$", "", basename(RepeatMaskerFile)), ".GRanges", ".Rdata"))
  ...
  GRdata <- GRanges(seqnames=gsub("chr", "", unlist(repeatMaskData[,5])),
                    ranges=IRanges(start=unlist(repeatMaskData[,6]), end=unlist(repeatMaskData[,7])),
                    strand=gsub("C", "-", unlist(repeatMaskData[,9])),
